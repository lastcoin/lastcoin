...
uint256 hashGenesisBlock("0x000007fbba3483054fe422751f0c174c0ad0928a1798892132b4324917d0f476"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // lastcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "lastcoin Signed Message:\n";
...
{
    // lastcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // lastcoin: 1 day
static const int64 nTargetSpacing = 3 * 60; // lastcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // lastcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, lastcoin - DarkGravity v2, written by Evan Duffield - evan@lastcoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
// For LastCoin also add the protocol rule that the first output in the coinbase must go to the charity address and have at least 2.5% of the subsidy (as per integer arithmetic) (thx to einsteinium!)
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // lastcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // lastcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
hashGenesisBlock = uint256("0x0000003cb0fd2b4adbe5e2d4d42ed76bcddde5e7d6192f2c66fd2f68b466e356");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "07/June/2014 The Dawn of THE Global Currency!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1402571218;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 48924;

        if (fTestNet)
        {
            block.nTime    = 1402127931;
            block.nNonce   = 0;
        }
...
/**/
        ...
        assert(block.hashMerkleRoot == uint256("0xc0f17dfbe44e8764eff24dc103d013e4458fe3ae2715e47bc42eec3218fbb3e2"));
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb7, 0xde }; // lastcoin: increase each by adding 2 to bitcoin's value.
...
//    printf("%d\n", scriptPubKeyIn[0]);
    ...
    int64 nFees = 0;
    {
		...
		// With LastCoin, at least 2.5% of all of the block subsidy should go to the charity address. (thx to einsteinium)
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("lastcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("lastcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static lastcoinMiner(CWallet *pwallet)
{
    printf("lastcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lastcoin-miner");
    ...
    try { loop {
        ...
        printf("Running lastcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("lastcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&lastcoinMiner, pwallet));
}
